...
#### Stuff to get the chatbot working
@app.route('/compatible_unit/<unit>', methods=['GET', 'POST'])
def compatible_unit(unit):
    data_in = request.get_json()
    categories = list(map(lambda x: x["uos_code"], calculate_categories(data_in['units_taken'])['Can take']))

    return "You can take this unit now!" if unit in categories else "You cannot take this unit. Please double check the units you have completed", 200

# Make it work
if __name__ == "__main__":

    # True -> run the server
    # False -> test the individual functions
    server = True

    show_necessary_sufficient = False
    show_graph = False
    ...
    if server:
        ...
        # Testing the functions with some hard-coded inputs to make sure it gives the expected result
        if show_necessary_sufficient:
        ...
        courses_taken = ["INFO1110", "COMP2129"]
        ...
        # INFO1110 -> already completed
        # INFO1910 -> prohibited
        # DATA2901 -> no prereq
        # COMP2823 -> already satisfied requirements
        # INFO3333 -> complex dependency
        targets = ["INFO1110", "INFO1910", "DATA2901", "COMP2823", "INFO3333"]
